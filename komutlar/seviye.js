...
exports.run = async (client, msg, args) => { 
    ...
    var lvl = db.fetch(`seviye_${u.id + msg.guild.id}`); 
        ...
        let sira = '' 
        const sorted = msg.guild.members.filter(u => !u.user.bot).array().sort((a, b) => { 
        ...
        ctx.arc(257 + 18.5, 125.5 + 18.5 + 36.25, 18.5, 1.5 * Math.PI, 0.5 * Math.PI, true); 
        ctx.fill(); 
        ctx.fillRect(257 + 18.5, 125.5 + 36.25, xp * de, 37.5); ctx.arc(257 + 18.5 + xp * de, 125.5 + 18.5 + 36.25, 18.75, 1.5 * Math.PI, 0.5 * Math.PI, false); 
        ctx.fill(); 
        ...
        ctx.textAlign = "left"; 
        ctx.fillText(`${u.tag}`, 225, 150) 
        ctx.beginPath(); 
        ctx.lineWidth = 8; 
        ctx.fill() 
        ctx.lineWidth = 8; 
        ctx.arc(43 + 70, 67 + 67, 67, 0, 2 * Math.PI, false); 
        ctx.clip(); 
        ctx.drawImage(avatar, 43, 69, 142, 130); 
