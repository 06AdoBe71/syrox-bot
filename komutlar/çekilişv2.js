...
if(!args[0]) return message.channel.send(`Bir eylem yazmalısınız \n **s!çekiliş başlat** veya **s!çekiliş tekrar** `)
...
message.channel.send(`Çekilişin ödülünü yazın.`).then(() => {
...
message.channel.send(`Çekilişin yapılacağı kanalı etiketleyin.`).then(() => {
message.channel.awaitMessages(filter, { maxMatches: 1, time: 60000, errors: ['time'] }).then(collected => {
...
message.channel.send(`Çekilişin süresini yazın. (5 saniye/5 dakika/5 saat)`).then(() => {
message.channel.awaitMessages(filter, { maxMatches: 1, time: 60000, errors: ['time'] }).then(collected => {
